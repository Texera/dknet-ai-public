  ...
  template: `
    <table class="ant-table">
      <tbody>
        ...
        <ng-container *ngIf="unit.computingUnit.type === 'aws'; else nonAwsRows">
          <tr>
            <th>Instance Type</th>
            <td>{{ awsResource["instanceType"] || "N/A" }}</td>
          </tr>
          <tr>
            <th>Region</th>
            <td>{{ awsResource["region"] || "N/A" }}</td>
          </tr>
          <tr>
            <th>Instance ID</th>
            <td>{{ awsResource["instanceId"] || "N/A" }}</td>
          </tr>
        </ng-container>
        <ng-template #nonAwsRows>
          <tr>
            <th>CPU Limit</th>
...
export class ComputingUnitMetadataComponent {
  ...
  readonly awsResource = this.unit.computingUnit.resource as unknown as Record<string, unknown>;
}
